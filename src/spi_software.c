                      ...
                      , uint8_t len, uint8_t *data)
{
    ...
    if (num_values < 1)
        shutdown("Invalid spi transfer: not enough data for at least 1 value (len*8 < width) len=%d width=%d", len, ss->width);
    
    if (extra_bits > 7)
        shutdown("Invalid spi transfer: unused data at the end (len > required bytes) len=%d width=%d", len, ss->width);
